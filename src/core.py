...
def generate_multivariate_series(n_series: int=10, n_timesteps: int=100, time_end: float=10, noise_std: float=0.3, seed: int=42) -> Tuple[np.ndarray, np.ndarray]:
    ...
    data = np.array([np.sin(time + phase) + np.random.normal(0, noise_std, n_timesteps) for phase in phases])
    return (data, time)
...
def plot_reconstruction_comparison(data: np.ndarray, reconstructed: np.ndarray, time: np.ndarray, n_series: int=3, output_path: Path | None=None, plot: bool = False):
    ...
    for i in range(n_series):
        ...
        axes[i].plot(time, reconstructed[i], label='Reconstructed', color='#D4A574', linewidth=1.2, linestyle='--')
        axes[i].set_xlabel('Time')
        axes[i].set_ylabel('Value')
        axes[i].legend(loc='best')
    plt.suptitle('Original vs Reconstructed Time Series (SVD)', fontsize=12, y=0.98, color='0.2')
    if output_path:
